...
class MultiModelForm extends CWidget
{
   ...
   /**
	...
	error_log(print_r($_POST,true));
        ...
        $deleteItems = array();
	error_log(print_r(get_class($this->model),true));
		...
		error_log(print_r('in initItems',true));

error_log(print_r($formData[$modelClass],true));
        if (isset($formData[$modelClass]['n__']))
        {
//		error_log(print_r($formData[$modelClass]['n__'],true));
            foreach ($formData[$modelClass]['n__'] as $idx => $attributes)
            {
                $model = new $modelClass;
                $model->attributes = $attributes;
				error_log(print_r($model,true));
if(method_exists($model,'uploadFile')){
$model->uploadFile();
error_log(print_r('cè il metodo uploadfile',true));
}
                if (!empty($masterValues))
...
//error_log(print_r($model,true));
        ...
        if (!empty($formData[$modelClass])) //has cloned elements
        {
			...
			error_log('formData[$modelClass]: '.print_r($formData[$modelClass],true));
				error_log('refarray: '.print_r($refArray,true));
				error_log('refAttribute: '.print_r($refAttribute,true));
            ...
            if (!empty($refArray))
				...
				error_log(print_r('sono nell ultimo prima ciclo',true));
				error_log(print_r($formData[$modelClass],true));

                    foreach ($formData[$modelClass] as $attrKey => $values)
                    {
									error_log(print_r($values[$idx],true));
						...
						if ($model->MultiModelForm['elements'][$attrKey]['type']=='file'){
										error_log(print_r('sto gestendo il file',true));
										$behaviors = $model->behaviors();
										if (!empty($behaviors['NUploadFile']))
$model->uploadFile($attrKey.'['.$idx.']');
						}
                    }
...
//
				error_log(print_r('sono nell ultimo',true));
//if(method_exists($model,'uploadFile')){
//$model->uploadFile();
error_log(print_r('cè il metodo uploadfile',true));
//}
